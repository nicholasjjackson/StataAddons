program define regreport
syntax varlist [if] [pweight] [, iv(varlist) covars(string) error(name) star cat(numlist max=1 integer) estround(numlist max=1 integer) log vce(string) ]
	...
	foreach var of varlist `varlist' {
		...
		*For categorical Predictors
		...
		if `num' <=`cutpoint' {
			...
			if "`weight'" !=""	{
				regress `var' i.`iv' `covars' $_if [`weight' `exp'], vce(`vcetype')
			}
			else	{
				regress `var' i.`iv' `covars' $_if , vce(`vcetype')
			}
					capture drop resid
		...
		*For Continuous Predictors
			...
			if "`weight'" !=""	{
				regress `var' `iv' `covars' $_if [`weight' `exp'], vce(`vcetype')
			}
			else	{
				regress `var' `iv' `covars' $_if , vce(`vcetype')
			}
