...
program cgeeinteract3d
syntax varlist (max=1) [if] [pweight] [, iv1(varname) iv2(varname) covars(string)  fam(name) link(name) iterate(numlist max=1 integer) id(varname) time(varname) corr(name)]
set more off
...
if "`weight'" == "" {
	...
	if "`corr'" !="" {
		xtgee `varlist' c.`iv1'##c.`iv2' `covars', family(`fam') link(`link')  iterate(`itnum') corr(`corr')
	}
	else {
		xtgee `varlist' c.`iv1'##c.`iv2' `covars', family(`fam') link(`link')  iterate(`itnum')
		}
}
if "`weight'" != "" {
	if "`corr'" !="" {
		xtgee `varlist' c.`iv1'##c.`iv2' `covars' [`weight'`exp'], family(`fam') link(`link')  iterate(`itnum') corr(`corr')
	}
	else {
		xtgee `varlist' c.`iv1'##c.`iv2' `covars' [`weight'`exp'], family(`fam') link(`link')  iterate(`itnum') 
	}
}
