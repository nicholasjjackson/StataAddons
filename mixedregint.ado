program define mixedregint
syntax varlist [if] [in] [pweight]  [, iv1(varlist) iv2(varlist) covars(string) log  error(name) star cat(numlist max=1 integer) estround(numlist max=1 integer) id(varname) time(varname) COVariance(name) full]
...
foreach iv_2 in `iv2' {
	use `faketemp', clear
		...
		if `num1' <=`cutpoint' & `num2' <=`cutpoint' {
			...
			if "`weight'" !=""	{
				if "`covariance'" !="" {
					xtmixed `var' `iv_1'##`iv_2' `covars' $_if [`weight' `exp'], ||`id':`time', covariance(`covariance') reml
						testparm `iv_1'#`iv_2'
							local pint=r(p)
					xtmixed `var' i.`iv_1' i.`iv_2' `covars' $_if [`weight' `exp'], ||`id':`time', covariance(`covariance') reml
				}
				if "`covariance'" =="" {
					xtmixed `var' `iv_1'##`iv_2' `covars' $_if [`weight' `exp'], ||`id':`time', reml
						testparm `iv_1'#`iv_2'
							local pint=r(p)
					xtmixed `var' i.`iv_1' i.`iv_2' `covars' $_if [`weight' `exp'], ||`id':`time', reml
				}
			}
			
			else	{
				if "`covariance'" !="" {
					xtmixed `var' `iv_1'##`iv_2' `covars' $_if , ||`id':`time', covariance(`covariance') reml
						testparm `iv_1'#`iv_2'
							local pint=r(p)
					xtmixed `var' i.`iv_1' i.`iv_2' `covars' $_if , ||`id':`time', covariance(`covariance') reml
				}
				if "`covariance'" =="" {
					xtmixed `var' `iv_1'##`iv_2' `covars' $_if , ||`id':`time', reml
						testparm `iv_1'#`iv_2'
							local pint=r(p)
					xtmixed `var' i.`iv_1' i.`iv_2' `covars' $_if, ||`id':`time', reml
				}
			}
					*capture drop resid
		...
		if `num1' <=`cutpoint' & `num2' >`cutpoint' {
			...
			if "`weight'" !=""	{
				if "`covariance'" !="" {
					xtmixed `var' `iv_1'##c.`iv_2' `covars' $_if [`weight' `exp'], ||`id':`time', covariance(`covariance') reml
						testparm `iv_1'#c.`iv_2'
							local pint=r(p)
					xtmixed `var' i.`iv_1' `iv_2' `covars' $_if [`weight' `exp'], ||`id':`time', covariance(`covariance') reml
				}
				if "`covariance'" =="" {
					xtmixed `var' `iv_1'##c.`iv_2' `covars' $_if [`weight' `exp'], ||`id':`time', reml
						testparm `iv_1'#c.`iv_2'
							local pint=r(p)
					xtmixed `var' i.`iv_1' `iv_2' `covars' $_if [`weight' `exp'], ||`id':`time', reml
				}				
			}
			else	{
				if "`covariance'" !="" {
					xtmixed `var' `iv_1'##c.`iv_2' `covars' $_if , ||`id':`time', covariance(`covariance') reml
						testparm `iv_1'#c.`iv_2'
							local pint=r(p)
					xtmixed `var' i.`iv_1' `iv_2' `covars' $_if , ||`id':`time', covariance(`covariance') reml
				}
				if "`covariance'" =="" {
					xtmixed `var' `iv_1'##c.`iv_2' `covars' $_if , ||`id':`time', reml
						testparm `iv_1'#c.`iv_2'
							local pint=r(p)
					xtmixed `var' i.`iv_1' `iv_2' `covars' $_if , ||`id':`time', reml
				}
			}
					*capture drop resid
					*predict resid if e(sample), pearson
		...
		if `num1' > `cutpoint' & `num2' <=`cutpoint' {
			...
			if "`weight'" !=""	{
				if "`covariance'" !="" {
					xtmixed `var' `iv_2'##c.`iv_1' `covars' $_if [`weight' `exp'], ||`id':`time', covariance(`covariance') reml
						testparm `iv_2'#c.`iv_1'
							local pint=r(p)
					xtmixed `var'  `iv_1' i.`iv_2' `covars' $_if [`weight' `exp'], ||`id':`time', covariance(`covariance') reml
				}
				if "`covariance'" =="" {
					xtmixed `var' `iv_2'##c.`iv_1' `covars' $_if [`weight' `exp'], ||`id':`time',  reml
						testparm `iv_2'#c.`iv_1'
							local pint=r(p)
					xtmixed `var'  `iv_1' i.`iv_2' `covars' $_if [`weight' `exp'], ||`id':`time', reml
				}
			}
			else	{
				if "`covariance'" !="" {
					xtmixed `var' `iv_2'##c.`iv_1' `covars' $_if , ||`id':`time', covariance(`covariance') reml
						testparm `iv_2'#c.`iv_1'
							local pint=r(p)
					xtmixed `var' `iv_1' i.`iv_2'  `covars' $_if , ||`id':`time', covariance(`covariance') reml
				}
				if "`covariance'" =="" {
					xtmixed `var' `iv_2'##c.`iv_1' `covars' $_if , ||`id':`time', reml
						testparm `iv_2'#c.`iv_1'
							local pint=r(p)
					xtmixed `var' `iv_1' i.`iv_2'  `covars' $_if , ||`id':`time', reml
				}
			}
					*capture drop resid
					*predict resid if e(sample), pearson
		...
		if `num1' > `cutpoint' & `num2' >`cutpoint' {
			...
			if "`weight'" !=""	{
				if "`covariance'" !="" {
					xtmixed `var' c.`iv_1'##c.`iv_2' `covars' $_if [`weight' `exp'], ||`id':`time', covariance(`covariance') reml
						testparm c.`iv_1'#c.`iv_2'
							local pint=r(p)
					xtmixed `var' `iv_1' `iv_2' `covars' $_if [`weight' `exp'], ||`id':`time', covariance(`covariance') reml
				}
				if "`covariance'" =="" {
					xtmixed `var' c.`iv_1'##c.`iv_2' `covars' $_if [`weight' `exp'], ||`id':`time', reml
						testparm c.`iv_1'#c.`iv_2'
							local pint=r(p)
					xtmixed `var' `iv_1' `iv_2' `covars' $_if [`weight' `exp'], ||`id':`time', reml
				}	
				...
				if "`covariance'" !="" {
					xtmixed `var' c.`iv_1'##c.`iv_2' `covars' $_if , ||`id':`time', covariance(`covariance') reml
						testparm c.`iv_1'#c.`iv_2'
							local pint=r(p)
					xtmixed `var' `iv_1' `iv_2' `covars' $_if , ||`id':`time', covariance(`covariance') reml
				}
				if "`covariance'" =="" {
					xtmixed `var' c.`iv_1'##c.`iv_2' `covars' $_if , ||`id':`time', reml
						testparm c.`iv_1'#c.`iv_2'
							local pint=r(p)
					xtmixed `var' `iv_1' `iv_2' `covars' $_if , ||`id':`time', reml
				}
