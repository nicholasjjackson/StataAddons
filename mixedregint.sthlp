{smcl}
Mixed Effects Linear Regression Interaction Program (Restricted Maximum Likelihood Estimation)
Stata Verison 11
...
{title:Invocation:}
	mixedregint varlist [if] [pweight] [, iv1(varlist) iv2(varlist) covars(string) id(varname) time(varname) log error(name) star cat(integer) estround(integer) COVariance(name) full]

{title:Purpose:} Mixed Effects Linear Regression Interaction Program (Restricted Maximum Likelihood Estimation)
