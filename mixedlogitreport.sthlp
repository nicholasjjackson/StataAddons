{smcl}
Mixed Effects Logistic Regression Report Program (Maximum Likelihood)
Stata Verison 12
Code Version 1.0 03/12/2012 Nick Jackson, Biostatistician, University of Pennsylvania


{title:Invocation:}
	mixedlogitreport varlist [if] [pweight] [,  iv(varlist) covars(string) id(varname) time(varname) or error(name) star cat(numlist max=1 integer) estround(numlist max=1 integer)  COVariance(name)]

{title:Purpose:}  Mixed Effects Logistic Regression using Maximum Likelihood estimation.
