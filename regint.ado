program define regint
syntax varlist [if] [pweight]  [, iv1(varlist) iv2(varlist) covars(string) log error(name) star cat(numlist max=1 integer) estround(numlist max=1 integer) full vce(string)]
*Version 1.1 Created on 03/12/2012
*Version 1.0 03/12/2012: prgram created based upon version 4.0 of glmint.ado
*Version 1.1 04/05/2012-Removed Specification of Pearson Residuals, Allows i. in covars list
	...
	inspect `iv_2'
		...
		*Set the Cut Point
		...
		if `num1' <=`cutpoint' & `num2' <=`cutpoint' {
			...
			if "`weight'" !=""	{
				regress `var' `iv_1'##`iv_2' `covars' $_if [`weight' `exp'], vce(`vcetype')
					testparm `iv_1'#`iv_2'
						local pint=r(p)
				regress `var' i.`iv_1' i.`iv_2' `covars' $_if [`weight' `exp'], vce(`vcetype')
			}
			else	{
				regress `var' `iv_1'##`iv_2' `covars' $_if , vce(`vcetype')
					testparm `iv_1'#`iv_2'
						local pint=r(p)
				regress `var' i.`iv_1' i.`iv_2' `covars' $_if , vce(`vcetype')
			}
					capture drop resid
		...
		if `num1' <=`cutpoint' & `num2' >`cutpoint' {
			...
			if "`weight'" !=""	{
				regress `var' `iv_1'##c.`iv_2' `covars' $_if [`weight' `exp'], vce(`vcetype')
					testparm `iv_1'#c.`iv_2'
						local pint=r(p)
				regress `var' i.`iv_1' `iv_2' `covars' $_if [`weight' `exp'], vce(`vcetype')
			}
			else	{
				regress `var' `iv_1'##c.`iv_2' `covars' $_if , vce(`vcetype')
					testparm `iv_1'#c.`iv_2'
						local pint=r(p)
				regress `var' i.`iv_1' `iv_2' `covars' $_if , vce(`vcetype')
			}
					capture drop resid
					predict resid if e(sample)
					sum resid $_if , detail
		...
		if `num1' > `cutpoint' & `num2' <=`cutpoint' {
			...
			if "`weight'" !=""	{
				regress `var' `iv_2'##c.`iv_1' `covars' $_if [`weight' `exp'], vce(`vcetype')
					testparm `iv_2'#c.`iv_1'
						local pint=r(p)
				regress `var'  `iv_1' i.`iv_2' `covars' $_if [`weight' `exp'], vce(`vcetype')
			}
			else	{
				regress `var' `iv_2'##c.`iv_1' `covars' $_if , vce(`vcetype')
					testparm `iv_2'#c.`iv_1'
						local pint=r(p)
				regress `var' `iv_1' i.`iv_2'  `covars' $_if , vce(`vcetype')
			}
					capture drop resid
					predict resid if e(sample)
					sum resid $_if , detail
		...
		if `num1' > `cutpoint' & `num2' >`cutpoint' {
			...
			if "`weight'" !=""	{
				regress `var' c.`iv_1'##c.`iv_2' `covars' $_if [`weight' `exp'], vce(`vcetype')
					testparm c.`iv_1'#c.`iv_2'
						local pint=r(p)
				regress `var' `iv_1' `iv_2' `covars' $_if [`weight' `exp'], vce(`vcetype')
			}
			else	{
				regress `var' c.`iv_1'##c.`iv_2' `covars' $_if , vce(`vcetype')
					testparm c.`iv_1'#c.`iv_2'
						local pint=r(p)
				regress `var' `iv_1' `iv_2' `covars' $_if , vce(`vcetype')
			}
